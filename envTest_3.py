...
class envTestEnv(gym.Env):
    def __init__(self):
        ...
        # self.para = PARA
        self.para_val = 150
        self.rand_val = random.randint(-3, 3)
        # self.ipc = 0
        self.action_space = spaces.Discrete(2)
        self.observation_space = spaces.Discrete(4)
        self.x = 150
        self.state = self.para1[self.para_val]+self.rand_val

    def step(self, action):
        assert self.action_space.contains(action), "%r (%s) invalid" % (action, type(action))
        # self.para_val = self.x
        #self.state = self.para1[self.para_val]
        next_ipc = self.para1[self.para_val]+self.rand_val
        if action == 0:
        ...
        elif next_ipc < self.para1[self.para_val]+self.rand_val:
            reward = 100
            self.next_state = self.para1[self.para_val]+self.rand_val
            done = False
        elif next_ipc > self.para1[self.para_val]+self.rand_val:
            reward = -100
            self.next_state = self.para1[self.para_val] +self.rand_val
